...
import re
import logging
from utils.constants import anglrMId
...
def block_to_feature(data, idp, blocks=False, overlap_blocks=False, orthogonal=False):
    ...
    if blocks or overlap_blocks:
        for bt in range(len(idp.bond_types)):
            ...
            ijR = np.concatenate([b_edge_index.T+start_id, b_edge_cell_shift], axis=1).astype(np.long).tolist()
            rev_ijR = np.concatenate([b_edge_index[[1, 0]].T+start_id, -b_edge_cell_shift], axis=1).astype(np.long).tolist()
